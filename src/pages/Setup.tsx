...
export function Setup({ state, dispatch, onFinish }: SetupProps) {
  const t = useTheme();
    ...
    <IonPage>
      <IonContent fullscreen className="ion-padding" style={{ fontFamily: fontSans, '--background': t.bg, color: t.text } as React.CSSProperties}>
        <div className="ion-content-inner">
          <div style={{ textAlign: 'center', padding: '14px 0 6px' }}>
            ...
            {info.phase === 'pre' && (
              ...
              >
                ...
                <p style={{ fontSize: 15, fontWeight: 700, color: t.orange, margin: '4px 0 2px' }}>
                  باقي {info.daysTo} أيام على رمضان!
                </p>
                <p style={{ fontSize: 11, color: t.muted, margin: 0 }}>جهّز قلبك وجدولك من دلوقتي</p>
              </div>
            )}
          </div>
          ...
          <Card style={{ marginTop: 14 }} glow>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
              {[
                ...
                <button
                  key={i}
                  onClick={() => dispatch({ type: 'SET_PAGES', v: Math.max(1, state.dailyPages + b.v) })}
                  style={{
                    width: b.w,
                    height: b.w,
                    borderRadius: 11,
              ...
              <div style={{ textAlign: 'center', minWidth: 60 }}>
                <span style={{ fontSize: 48, fontWeight: 800, color: t.gold, lineHeight: 1 }}>
                  {state.dailyPages}
                </span>
                <p style={{ color: t.muted, fontSize: 10, margin: '2px 0 0' }}>صفحة/يوم</p>
              </div>
              {[
                { l: '+', v: 1, w: 34 },
                { l: '+١٠', v: 10, w: 40 },
                ...
                <button
                  key={i}
                  onClick={() => dispatch({ type: 'SET_PAGES', v: state.dailyPages + b.v })}
                  style={{
                    width: b.w,
                    height: b.w,
                    borderRadius: 11,
            ...
            >
              {k >= 1 ? (
                <span style={{ fontSize: 13, color: t.green, fontWeight: 700 }}>🎉 {k} ختمة في رمضان!</span>
              ) : (
                <span style={{ fontSize: 12, color: t.orange }}>📌 تحتاج ٢١ صفحة يومياً لختمة</span>
              )}
            </div>
          </Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {state.readingTimes.map((rt, i) => (
                ...
                >
                  ...
                  <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: t.text }}>{rt.label}</span>
                  ...
                  <button
                    onClick={() => {
                      if (nt.trim()) {
                        dispatch({
                          type: 'ADD_TIME',
                          t: { label: nt.trim(), icon: ICONS[state.readingTimes.length % ICONS.length]! },
                        });
                        setNt('');
                        setShowTI(false);
                      }
                ...
                >
                  <div
                    style={{
                      ...
                      borderRadius: 10,
                      background: '#fff',
                      transform: state.goals[g.key] ? 'translateX(0)' : 'translateX(20px)',
                      transition: 'all .3s',
          ...
          <Btn
            onClick={onFinish}
            style={{ width: '100%', marginTop: 18, fontSize: 16, padding: '15px' }}
          >
